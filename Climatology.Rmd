---
title: "Environmental Factors Related Tornado Energy on the Most Prolific Tornado Days in the Largest Tornado Groups"
author: "Zoe Shroder"
date: "7/31/2018"
...
Q: What are the broad geographic variations in CAPE and shear associated with tornado groups? How widely do these environmental characteristics vary geographically? Q: Which environmental variable (CAPE or shear) best explains group-level tornado energy? How are the environmental variables related to casualties?
...
Filter tornadoes to remove those not part of a large group. Group by group number and convective dates. Remove days within big groups (group days) having fewer than 10 tornadoes.
```{r}
...
ggplot(df, aes(y = Year)) +
  ...
  scale_size(range = c(1, 5), name = "Energy\nDissipation (GW)", labels = c("10", "100", "1000", "10,000", "100,000"), breaks = c(10, 11, 12, 13, 14)) +
...
df %>%
  ...
  mutate(CAPECIN = scale(annualMeanCAPEm * annualMeanCINm)) %>%
  ggplot(., aes(x = Yr, y = CAPECIN)) +
    scale_y_continuous() +
    geom_point() +
    geom_smooth(method = lm) +
...
model0a <- lmer(log(GroupDayMeanED) ~ scale(aHLCY) + scale(mCAPE) * scale(mCIN) + scale(Yr) + (1|Mo), weights = nT, data = df)
summary(model0a)

model1 <- lmer(log(GroupDayCas + 1) ~ scale(aHLCY) + scale(mCAPE) * scale(mCIN) + scale(Yr) + (1|Mo), weights = nT, data = df)
summary(model1)

model2 <- lmer(log(GroupDayCas + 1) ~ scale(aHLCY) + scale(mCAPE) + (1 | Mo), weights = nT, data = df)
summary(model2)
```
...
Energy dissipation
```{r}
ggplot(df, aes(x = GroupDayMeanED/10^9, y = preED/10^9, color = log10(GroupDayCas + 1))) +
  scale_color_continuous(guide=FALSE) +
         geom_point() + geom_smooth(method = lm, color = "red", se = FALSE) +
         geom_abline(slope = 1) +
         scale_x_log10(limits = c(.1, 3000), breaks = c(.1, 1, 10, 100, 1000), labels = c(".1", "1", "10", "100", "1000")) +
         scale_y_log10(limits = c(.1, 3000), breaks = c(.1, 1, 10, 100, 1000), labels = c(".1", "1", "10", "100", "1000")) +
  ylab("Predicted Energy Dissipation (GW)") + xlab("Average Energy Dissipation (GW)") +
  theme_minimal()
```
