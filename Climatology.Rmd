...
Q: What are the broad geographic variations in CAPE and shear associated with tornado groups? How widely do the environmental characteristics of groups vary geographically?

Q: Which environmental variable (CAPE or shear) best explains group-level average tornado energy? Which variable best explains worst-case group-level tornado energy?
...
Determine tornadoes that are close to each other in space and time. First project the lat/lon geometry. See https://epsg.io/102004.  Then extract the coordinates of the start locations as a N by 2 matrix of numbers, where N is the number of tornadoes. Also extract the date-time as a vector of class `POSIXct`.
```{r}
...
BigDaysInLargeGroups.sfdfT <- Tor.sfdfT %>%
  ...
  summarize(nT = n(),
            ...
            cDate1 = first(cDate)) %>%
  filter(nT >= 10) %>%
  mutate(Year = year(cDate1),
         Mo = month(cDate1),
         Month = format(cDate1, "%m"), # this is needed to preserve the leading zeros
         Day = format(cDate1, "%d"))
```
...
Extract the geographic centroids of tornado genesis on big tornado days by creating an `sf` object using `st_centroid()` that reduces the MULTIPOINT geometry to a POINT geometry. Compute the area of the group and the number of tornadoes per area (intensity).
```{r}
...
groupDayCentroids.sfdfT$groupIntensity <- groupDayCentroids.sfdfT$nT/groupDayCentroids.sfdfT$groupArea
```
