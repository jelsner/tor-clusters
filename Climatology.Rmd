...
```{r}
 rb <- brick(paste0("/Users/zoeschroder/Desktop/Projects/Tor-clusters/merged_AWIP32.20110427/merged_AWIP32.2011042709")) #<-- this is for varying NARR times
  CAPE <- raster(rb, layer = 375)
  HLCY <- raster(rb, layer = 323)
  CIN <- raster(rb, layer = 376)
```
...
sfdfTll <- st_transform(April27, crs = "+init=epsg:4326")
...
tm_shape(cape.r) +
  ...
  tm_format("World", legend.position = c("left", "top"),
                   attr.position = c("left", "top"),
                   legend.frame = FALSE,
                   inner.margins = c(.1, .05, .05, .05),
                    legend.text.size = 1.25, legend.width = -0.3, legend.title.size=2, legend.bg.color = "white") +
tm_shape(counties) +
  tm_borders(col = "grey") +
  tm_scale_bar(width = 0.5, size = 1.5, position = c("right", "bottom")) +
tm_shape(states) +
  tm_borders() +
tm_shape(April27, is.master = TRUE, projection = "merc") +
  tm_borders(col = "black", lwd = 3)  +
tm_shape(April27centroid) +
  tm_symbols(size = 1.25, col = "black", shape = 24) 

tm_shape(hlcy.r) +
  tm_raster(title = "HLCY (m^2/s^2)", n = 5) +
  tm_format("World", legend.position = c("left", "top"),
                   attr.position = c("left", "top"),
                   legend.frame = FALSE,
                   inner.margins = c(.1, .05, .05, .05),
