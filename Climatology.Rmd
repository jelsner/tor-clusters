...
model0a <- lmer(log(GroupDayTotalED) ~ scale(aHLCY) + scale(mCAPE) * scale(mCIN) + scale(mUSTM) + scale(Yr) + (1|Mo), weights = nT, data = df)
summary(model0a)
...
ggplot(df, aes(x = GroupDayTotalED/10^9, y = preED/10^9, color = log10(GroupDayCas + 1))) +
  ...
  ylab("Predicted Energy Dissipation (GW)") + xlab("Accumulated Energy Dissipation (GW)") +
  theme_minimal()
```
