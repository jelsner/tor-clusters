...
There are 9 groups lasting only one day, 64 groups lasting two days, etc.
...
```{r}
grid.newpage()
draw.pairwise.venn(15, 13, 10, category = c("Schroder", "Forbes"), lty = rep("blank", 
    2), fill = c("grey87", "grey61"), alpha = rep(0.5, 2), cat.pos = c(0, 
...
```{r}
grid.newpage()
draw.pairwise.venn(9, 9, 4, category = c("Schroder", "Fuhrmann"), lty = rep("blank", 
    2), fill = c("grey87", "grey61"), alpha = rep(0.5, 2), cat.pos = c(0, 
    0), cat.dist = rep(0.025, 2))
```
...
```{r}
ggplot(FPFN, aes(Cuttree)) + 
  geom_line(aes(y = False.Negative.Match, colour = "False Negative")) +
  geom_line(aes(y = False.Positive.Match, colour = "False Positive")) +
  labs(x = "Space - Time Units", 
       y = "% Match", 
       colour = "Match") +
  theme_bw() +
...
```
50% of all big days (>= 10 tornadoes) occur within a big group/outbreak (>= 30 tornadoes)
...
df %>%
  ...
  ggplot(., aes(x = Yr, y = annualMeanVSTMm)) +
    scale_y_continuous() +
    geom_point() +
    geom_smooth(method = lm) +
...
model2 <- lmer(log(GroupDayCas + 1) ~ scale(aHLCY) + scale(mCAPE) + scale(mUSTM) + (1 | Mo), weights = nT, data = df)
summary(model2)
```

Observed versus predicted.
```{r}
df$preED = exp(predict(model0a))
df$preCas = exp(predict(model2)) - 1
```
