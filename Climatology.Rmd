...
Our interest centers on clusters that are not too small (e.g., a family of tornadoes from a single supercell) and not too large (e.g., all tornadoes during a week). So we stop grouping once there are no additional pairs within a $\delta_k$\ of 50K ($\sim$14 space-time hours). This results in 6,156 unique clusters and 155 large (at least 30 tornadoes) clusters. The largest cluster occurred from April 26--28, 2011. It contains 293 tornadoes. Duration of the clusters ranges from 46 one-day clusters to one five-day cluster (Table~\ref{GroupsbyDuration}). Multi-day clusters account for 69.5\% of all our clusters. The cluster with the longest duration occurred from September 4--8, 2004 and contained 103 tornadoes (Fig.~\ref{LargestGroup}). Our clusters match the outbreaks identified subjectively by \cite{Forbes2006} with an agreement rate of 88\%.
...
tm_shape(longestgroup, projection = "merc", is.master = TRUE) + 
  ...
  tm_scale_bar(width = .6, size = 1.5, lwd = 2, color.dark = "gray70") +
  tm_compass(size = 7, lwd = 2, fontsize = 1.2, color.dark = "gray70") + 
    tm_format("World", legend.position = c("left", "top"),
                   ...
                   inner.margins = c(.04, .29, .1, .29)) +
  tm_layout(legend.bg.color = "white", legend.text.size = .75) +
tm_shape(longestdaytorns) +
  tm_symbols(size = 5, col = "ID", n = 5, palette = "seq", title.col = "September", labels = c("8th", "7th", "6th", "5th", "4th")) + 
  tm_layout(legend.title.size = 2.1,
            legend.position = c("right", "bottom"), 
            legend.stack = "horizontal",
            legend.frame = FALSE, 
            legend.text.size = 1.8, legend.width = -0.3, aes.palette = list(seq = "-Greys"))
```
...
as.data.frame(BigDays.sfdfT) %>%
  dplyr::select(cDate, nT, GroupDayCas, ATE) %>%
  top_n(n = 10, wt = nT) %>%
  arrange(desc(nT))
```
...
tm_shape(stateBorders) + 
  ...
  tm_scale_bar(width = .6, size = 1.5, lwd = 2, color.dark = "gray70") +
  tm_compass(size = 7, fontsize = 1.2, lwd = 2, color.dark = "gray70") + 
  tm_layout(legend.bg.color = "white", 
            legend.text.size = .75, 
            attr.position = c("right", "top"), 
            inner.margins = c(.1, .2, .1, .2)) +
#tm_shape(counties.sf) +
#  tm_borders(col = "gray40", alpha = .3) +
...
tm_shape(May30tornadoes) +
  tm_symbols(size = 5, col = "Hour2", palette = "Greys", title.col = "Time [CST]", labels = c("6 to 12", "12 to 18", "18 to 24", "0 to 6")) +
    tm_layout(legend.title.size = 1.8,
            legend.position = c("right", "bottom"), 
            legend.stack = "horizontal",
            legend.frame = FALSE, 
            legend.text.size = 1.6, legend.width = -0.2) +
...
tm_shape(stateBorders) + 
  tm_borders(col = "gray70") +
  tm_compass(size = 7, position = c("left", "bottom"), fontsize = 1.2, lwd = 2, color.dark = "gray70") + 
  tm_scale_bar(width = 0.6, size = 1.5, lwd = 2, color.dark = "gray70") +
#tm_shape(counties.sf) +
#  tm_borders(col = "gray40", alpha = .3) +
...
tm_shape(groupDayCentroids.sfdfT, is.master = TRUE, projection = "merc") +
  tm_symbols(col = "nT", n = 4, 
             ...
             size = "nT",
             shape =24, 
             scale = 4, legend.size.show = FALSE) + 
  tm_add_legend("symbol", title = "Count", shape = 24,
...
```{r, eval = FALSE}
BigDays.sfdfT %>%
  dplyr::select(cDate, maxCAPE, minCIN, maxHLCY, maxBS, ATE) %>%
  top_n(n = 3, wt = ATE) %>%
  arrange(desc(ATE))
...
```
`Figure 4: Environmental conditions at 12 UTC on May 6, 2003. The black line is the spatial extent of the tornado genesis locations and the black triangle is the centroid of the locations. The first tornado in the cluster started at 12:20 UTC. The black square indicates the locations of the highest value of CAPE (3660~J~kg$^{-1}$), the lowest value of CIN ($-$149~J~kg$^{-1}$), the highest value of helicity (308~m$^2$~s$^{-2}$) and the highest value of bulk shear (33~m~s$^{-1}$).`  CHECK THE Z TIME!!!!!
...
The regression model is best in the sense that it has the lowest Akaike Information Criterion (AIC) score, which measures the overall quality (goodness of fit and simplicity) of the model. Due to a large correlation between bulk shear and relative storm motion (0.59) we retain only bulk shear in the model. We determined that interactions between the environmental variables did not improve the model fit based on higher AIC scores when they were included. We also determined that {\it spatially averaged} values for the environmental variables makes the fit worse. The maximum (and minimum) values within the cluster area provide a better representation of the environmental conditions for the tornadoes on each big day because they are less contaminated by mesoscale and synoptic scale features.
...
ggplot() +
  geom_point(data = model1.under, aes(x = GroupDayTotalED/10^12, y = preATP/10^12, fill = log10(GroupDayCas + 1)), shape = 21, size = 8, show.legend = FALSE, stroke = 0) +
      scale_fill_gradient(low = "pink", high = "firebrick3") +
      geom_point(data = model1.over, aes(x = GroupDayTotalED/10^12, y = preATP/10^12, colour = log10(GroupDayCas + 1)),  size = 8, show.legend = FALSE, stroke = 0.5) +
        scale_colour_gradient(low = "lightskyblue", high = "blue") +
      geom_abline(slope = 1, size = 1.25) + 
      scale_x_log10(limits = c(1, 300),breaks = c(.01, .1, 1, 10, 100, 1000), labels = c(".01", ".1", "1", "10", "100", "1000")) +
      scale_y_log10(limits = c(1, 75), breaks = c(.01, .1, 1, 10, 100, 1000), labels = c(".01", ".1", "1", "10", "100", "1000")) +
      ylab("Estimated ATP [TW]") + xlab("ATP [TW]") +
...
tm_shape(stateBorders) + 
  ...
  tm_scale_bar(position = c("left", "bottom"), width = 0.5, size = 1.4, color.dark = "gray70") +
...
\item On average cluster severity increases by 124\% for every 10 m$^2$s$^{-2}$ increase in bulk shear holding the other variables constant.
\item On average cluster severity increases by 29\% for every 1000 J~kg$^{-1}$ increase in CAPE holding the other variables constant.
...
ggplot(., aes(x = Year, y = nBD)) +
    ...
    scale_color_gradient(low = "pink", high = "firebrick3", guide = "legend") + 
...
```
 Number of big days by year, 1994 – 2017. Points are sized and colored by annual average ATP.
