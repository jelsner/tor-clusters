...
```{r}
TorP.sfdf <- st_transform(Tor.sfdf, crs = 102004)
...
Add the group number to each tornado then create a new simple feature object that summarizes the tornadoes by group. Also, create a group-day identification. Combine the outbreak number and convective day with no spaces or symbols. (e.g., group number: 276; cDate: 1994-03-01 --> groupDay: 27619940301) 
```{r}
Tor.sfdf$groupNumber <- groupNumber
Tor.sfdf <- Tor.sfdf %>%
  mutate(groupDay = paste0(groupNumber, gsub("-", "", cDate)))
```

Compute group-level statistics. Keep only groups with at least 30 tornadoes.
```{r}
Group.sfdf <- Tor.sfdf %>%
...
Map the tracks of the top 10 groups by energy dissipation. First select the top 12 groups by total ED. Then create a simple features data frame using only the tornadoes in these groups by filtering on group number.
```{r}
df <- as.data.frame(Group.sfdf) %>%
