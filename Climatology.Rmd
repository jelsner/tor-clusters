...
tm_shape(stateBorders, is.master = FALSE) +
  tm_borders() 
```
** Figure 5: Big day density by county **
...
```
**Figure X: Number of big days by year, 1994 – 2017. Points are sized and colored by annual average ATP.**
...
#ggplot(pgridL, aes(x = maxCAPE2, y = maxBS2, fill = value)) +
 ...
 # scale_fill_viridis_c(breaks = log(10^e), labels = c(1, 3.2, 10, 32))
```
**Figure 6: Predictions of ATP across a range of CAPE and bulk shear values holding CIN, helicity, year, and month as constants.  **
...
```
**Figure 5: Conditional standardized residuals from the linear regression model. (A) Histogram and (B) Residuals as a function of predicted values of ATP. **
...
```
**Figure 8: Areas defining the boundary of all tornadoes on big days. Days selected are those where the model most over predicted (blue) and most under predicted (pink) ATP. **
