...
Tor.sfdf <- Tor.sfdf %>%
  ...
  mutate(DateTime = as.POSIXct(paste(yr, mo, dy, time), format = "%Y%m%d%H:%M:%S"),
         Hour = hour(DateTime),
         Year = year(DateTime),
         cDateTime = as_datetime(ifelse(Hour<6, (DateTime - 86400), DateTime), tz = Sys.timezone()),
...
Create unique ID for each outbreak and outbreak day. To do this combine the outbreak number and convective day with no spaces or symbols. (EX: outbreaknumber: 276; cDate: 1994-03-01 --> ID: 27619940301)
