...
#overpreddays <- BigDaysInLargeGroups.sfdfT %>%
#  filter(cDate %in% overdays)    **Returning nothing

overpredday1 <- BigDaysInLargeGroups.sfdfT %>%
  filter(cDate == "2014-02-20")
overpredday2 <- BigDaysInLargeGroups.sfdfT %>%
  filter(cDate == "1995-04-19")
overpredday3 <- BigDaysInLargeGroups.sfdfT%>%
  filter(cDate == "2000-05-18")
overpredday4 <- BigDaysInLargeGroups.sfdfT %>%
  filter(cDate == "2011-04-28")
overpred <- rbind(overpredday1,overpredday2,overpredday3,overpredday4)
```

Create a data frame of the under predicted big days. 
```{r}
# underdays <- c("2003-05-04", "2008-02-05", "2011-04-27") 
# underpreddays <- BigDaysInLargeGroups.sfdfT %>%
#  filter(cDate %in% overdays)    **Returning nothing

underpredday1 <- BigDaysInLargeGroups.sfdfT %>%
  filter(cDate == "2003-05-04")
underpredday2 <- BigDaysInLargeGroups.sfdfT %>%
  filter(cDate == "2008-02-05")
underpredday3 <- BigDaysInLargeGroups.sfdfT %>%
  filter(cDate == "2011-04-27")
underpred <- rbind(underpredday1,underpredday2,underpredday3)
```
...
```{r}
tm_shape(overpred) +
  tm_polygons(col = NA, lwd = 2, alpha = 0.2, border.col = "blue") + 
  tm_fill(col = "blue") +
tm_shape(underpred) +
  tm_polygons(col = NA, lwd = 2, alpha = 0.2, border.col = "magenta") + 
tm_shape(stateBorders, projection = "laea_NA", is.master = TRUE) + 
  tm_borders() +
  tm_compass() + tm_scale_bar() 
```
...
tm_shape(stateBorders) + 
  ...
  tm_layout(panel.labels = paste(test$cDate, test$work), panel.label.size = 1, panel.label.bg.color = "grey90") 
#the dates are wrong
```
