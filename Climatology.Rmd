...
```{r}
#save(BigDays.sfdfT, Groups.sfdfT, Torn.sfT, GroupTornadoes, file = "BigDays.RData")
load("BigDays.RData")
dim(BigDays.sfdfT)
```
...
```{r}
sum(BigDays.sfdfT$GroupDayCas)/sum(Torn.sfT$cas) * 100
sum(BigDays.sfdfT$GroupDayFat)/sum(Torn.sfT$fat) * 100
```

What is the percentage of all big days (>= 10 tornadoes) that occur within a big group?
```{r}
TotalBigDays <- Torn.sfT %>%
  group_by(groupNumber, cDate) %>%
  summarize(nT = n()) %>%
  filter(nT >= 10)
