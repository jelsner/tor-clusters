...
---
Spatial Analysis of Environmental Factors Related to Large Tornado Groups
=========================================================================
...
Why is this important? Tornado groups represent the most concentrated large-scale risk for damage and casualties. In fact, 3/4 of all fatalities occur on days with the most tornadoes within an outbreak.
...
Tor.sfdf <- Tor.sfdf %>%
  filter(yr >= 1994, #start in 1994 because of comprehensive radar
         st != c("AK", "PR", "HI")) %>% #remove tornadoes occurring in Alaska, Puerto Rico, and Hawaii
...
groupDayCentroids.sfdfT %>%
  ...
  ggplot(., aes(x = Year, y = as.numeric(avgDensity) * 10^10)) +
    ...
    geom_point() +
    geom_smooth(method = lm) +
    ylab("Group Density [Tornadoes/100 sq. km]") +
    theme_minimal()
```
...
Save the states and counties from the `USAboundaries` package. 
```{r}
us.sf <- us_states()
counties <- us_counties()
```

Set the CRS for counties to match the environmental data raster grids and the BigDaysInLargeGroups.sfdfT
```{r}
counties <- st_transform(counties, 
  crs = "+proj=lcc +lat_1=50 +lat_2=50 +lat_0=50 +lon_0=-107 +x_0=0 +y_0=0 +a=6371200 +b=6371200 +units=m +no_defs")
```

Convert counties and BigDaysInLargeGroups.sfdfT to Spatial objects
```{r}
counties <- as(counties, "Spatial")
hulls <- as(BigDaysInLargeGroups.sfdfT, "Spatial")
```

See how many outbreaks occur in each county across the US. Use the `gIntersects` function from the **rgeos** package. Set the rownames to the names of the counties. Create a gIvalues column where each value is the Sum of "TRUE" values in teh gI df. 
```{r}
gI <- gIntersects(hulls, counties, byid=TRUE)
rownames(gI) <- counties$name
colnames(gI) <- hulls$cDate

counties$gIvalues = rowSums(gI == "TRUE")
```

Remove Alaska, Hawaii, and Puerto Rico from the states df
```{r}
states <- us.sf %>% 
  filter(stusps != "AK",
         stusps != "PR",
...
Create a map of the outbreak density in each county in the United States. Color by the gIvalues column. 
```{r}
tm_shape(states) +
  tm_borders() +
tm_shape(counties) +
   tm_polygons("gIvalues",
              title = "Number of Outbreaks",
              palette = "Reds", n = 20) +
  tm_borders(col = "grey") +
  tm_compass() + 
  tm_scale_bar() +
  tm_format_World(legend.position = c("left", "bottom"),
                  attr.position = c("left", "bottom"),
                  inner.margins = c(.025, .025, .075, .025)) +
  tm_style_white("Outbreak Density by County",
                 frame = TRUE, title.size = 1.5, title.position = c("center", "TOP"))
```
