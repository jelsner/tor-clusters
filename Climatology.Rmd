...
test <- BigDays.sfdfT$ATE
descdist(test, boot = 1000)
```

```{r}
gammafit  <-  fitdistrplus::fitdist(test, "gamma")
weibullfit  <-  fitdistrplus::fitdist(test, "weibull")
lnormfit  <-  fitdistrplus::fitdist(test, "lnorm")  # not supported?

library(flexsurv) # on CRAN

gengammafit  <-  fitdistrplus::fitdist(test, "gengamma",
...
```{r}
library(car)
qqPlot(log(test)) ## cannot assume normality
qqPlot(sqrt(test))
qqPlot((test^0.3))
qqPlot(test, distribution = "gamma", shape = 3)
```
