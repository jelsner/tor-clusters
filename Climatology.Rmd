---
title: "Environmental Factors Related to the Most Prolific Tornado Days Within Large Tornado Groups"
author: "Zoe Shroder"
date: "7/31/2018"
...
df %>%
  ...
  ggplot(., aes(x = Yr, y = annualMeanHLCY)) +
    scale_y_continuous(limits = c(0, NA)) +
    geom_point() +
    geom_smooth(method = lm) +
...
model0a <- lmer(log(GroupDayMeanED) ~ scale(aHLCY) + scale(mCAPE) * scale(mCIN) + (1|Mo), weights = nT, data = df)
summary(model0a)

model0b <- lmer(log(GroupDayMaxED) ~ scale(aHLCY) + scale(mCAPE) * scale(mCIN) + (1|Mo), weights = nT, data = df)
summary(model0b)
