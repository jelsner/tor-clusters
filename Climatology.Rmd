...
Q: What is the geographic variation in the large tornado groups? Q: Which environmental variable (CAPE, shear) best explains accumulated tornado energy? Q: Which best explains casualties? 

Mortality rate is the rate of death in a population. Mortality rates for the population affected by tornado outbreaks. Mortality rates are generally expressed as the number of deaths per 1000 individuals per year.

Why is this important? Tornado groups represent a concentrated large-scale risk for significant damage and casualties. In fact 3/4 of all fatalities occur on days with the most tornadoes within a large 'outbreak'.

Q: Can we quantify the mortality rate per m/s increase in shear and per J/kg increase in CAPE?

This analysis is done at the scale of tornado groups rather than at more local scales associated with individual tornadoes. 
...
Introduce the term "Accumulated Tornado Energy (ATE)"
...
Remove tornadoes in Hawaii, Alaska, and Puerto Rico and those occurring before 1994. The year marks the beginning of comprehensive WSR-88D radar. For missing EF ratings use the modification rules (if/else) defined here: https://www.spc.noaa.gov/wcm/OneTor_F-scale-modifications.pdf
```{r}
Tor.sfdf <- Tor.sfdf %>%
  filter(yr >= 1994,
         !st %in% c("AK", "PR", "HI")) %>%
