...
```
XX% of all big days (>= 10 tornadoes) occur within a big group/outbreak (>= 30 tornadoes)
...
```{r}
#save(BigDays.sfdfT, Groups.sfdfT, BigDayTornadoes, GroupTornadoes, All_Tornadoes file = "BigDays.RData")
load("BigDays.RData")
dim(BigDays.sfdfT)
```
...
longestdaytorns <- GroupTornadoes %>%
  filter(groupNumber == 3075)
cDay1 <- longestdaytorns %>%
  filter(cDate == "2004-09-04")
cDay2 <- longestdaytorns %>%
  filter(cDate == "2004-09-05")
cDay3 <- longestdaytorns %>%
  filter(cDate == "2004-09-06")
cDay4 <- longestdaytorns %>%
  filter(cDate == "2004-09-07")
cDay5 <- longestdaytorns %>%
  filter(cDate == "2004-09-08")
```
...
tm_shape(stateBorders) + 
  ...
  tm_scale_bar(width = 10, size = 10, lwd = 3) +
  tm_layout(legend.bg.color = "white", legend.text.size = .75) +
tm_shape(counties.sf) +
  tm_borders(col = "gray40", alpha = .3) +
  tm_scale_bar(width = 10, size = 10) +
...
tm_shape(cDay1) +
  tm_symbols(size = 3, col = "yellow") + 
tm_shape(cDay2) +
  tm_symbols(size = 3, col = "blue") + 
tm_shape(cDay3) +
  tm_symbols(size = 3, col = "red") + 
tm_shape(cDay4) +
  tm_symbols(size = 3, col = "green") +
tm_shape(cDay5) +
  tm_symbols(size = 3, col = "magenta")
```
**Figure 1: The September 4 – 8, 2004 event has the duration of all tornado groups. The black line is the hull (spatial extent) of the entire group. Each dot is colored by a different big day. (Yellow: 9/4; Blue: 9/5; Red: 9/6; Green: 9/7; Pink: 9/8)**  
