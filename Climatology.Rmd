...
BigDays.sfdfT %>%
  ...
  summarize(count = n())
```
**Table X: Total number of big days associated with each Z time.** #CORRECT
...
```
**Figure 1: The September 4 – 8, 2004 event has the longest duration of all tornado groups. The black line is the hull (spatial extent) of the entire group. Each dot is colored by a different big day. **  #CORRECT
...
BigDays.sfdfT %>%
  ...
  arrange(desc(nT))
```
**Table 2: The top ten big days in the largest tornado groups. ATP is the accumulated tornado power on a big day. ** #CORRECT
...
BigDays.sfdfT %>%
  ...
  arrange(desc(ATP))
```
**Table 4: The maximum and minimum values associated with each big day. The top 3 rows represent the top three big days sorted by the number of tornadoes. The bottom three rows are the bottom 3 big days by the number of tornadoes. ** #CORRECTED
...
tm_shape(groupDayCentroids.sfdfT, is.master = TRUE, projection = "merc") +
  ...
  tm_layout(legend.title.size=1.8, 
            ...
            legend.width = -0.25)
``` 
**Figure 3: Centroids of genesis locations occurring on big days in large groups. The triangles are sized by the number of tornadoes on that day.** #CORRECT
...
BigDays.sfdfT %>%
  ...
  arrange(desc(nT))
```
**Table 5: Seasonal variation in ATP (TW), number of tornadoes, and the number of big days by month. The number of tornadoes and the number of big days is based on the period 1994 – 2017. ** #CORRECT
...
```
**Figure 7: Actual versus predicted accumulated tornado power (ATP) for the n = 212 big tornado days. The predicted are based on the regression model (Eq. 3). The color shading from dark to light indicates an increasing number of casualties. ** #CORRECT
