...
ggplot(df, aes(y = Year)) +
  ...
  scale_size(range = c(1, 5), name = "Accumulated\nTornado\nEnergy (ATE)\n[GW]", labels = c("10", "100", "1000", "10,000", "100,000"), breaks = c(10, 11, 12, 13, 14)) +
...
```{r}
cor.test(x = BigDaysInLargeGroups.sfdfT$ATE, 
         y = BigDaysInLargeGroups.sfdfT$nT, 
         method = 'spearman')
```
...
ggplot(., aes(x = Year, y = nBD)) +
    geom_point(aes(size = avgATE), color = "gray70", alpha = .5) +
    scale_size_continuous(name = "ATE [GW]") +
    scale_x_continuous(breaks = seq(1995, 2015, 5)) +
    xlab("") + ylab("Number of Big Days") +
    theme_minimal() 
```
...
ggplot(BigDaysInLargeGroups.sfdfT, aes(log10(GroupDayTotalED))) +
  geom_histogram(binwidth = .5, color = "white", fill = "gray70") +
...
Plots for the interaction term. NOT USED.
```{r}
library(interplot)
...
Locate specific values on the plot. NOT USED.
```{r}
# CIN at 250 J/kg
...
#         scale_color_viridis_c(guide = FALSE, direction = -1) +
         ...
         geom_point() + 
         geom_label_repel(aes(x = GroupDayTotalED/10^9, y = preATE/10^9, label = as.character(cDate)), color = "black", data = model1.df2, size = 2) +
         geom_abline(slope = 1) +
  ...
  theme_minimal()
```
**Figure 8: Actual versus predicted accumulated tornado energy (ATE) for the $n$ = 154 big tornado days. The predicted are based on the regression model. The color shading from dark to light indicates increasing number of casualties.**
...
p1 + p2 + plot_layout(ncol = 2, widths = c(2, 3))
```
