...
```{r}
library(lme4)
summary(lmer(log(GroupDayTotalED) ~  I(Yr - 2004) + (mCAPE2:mCIN2|Mo) + 
                mCAPE2 + mCIN2 + mUSTM2 + mHLCY2,
             weights = nT, 
             data = df))
...
summary(lm(log(GroupDayTotalED) ~  I(Yr - 2004) +
                     mCAPE2 * mCIN2 + mBS2,
              weights = nT, 
              data = df2))
...
summary(lm(log(GroupDayTotalED) ~ 
                     mCAPE2 + mCIN2 + mUSTM2,
              weights = nT, 
              data = df3))
...
model0a <- lmer(log(GroupDayTotalED) ~ I(mCAPE/1000) + I(Yr - 2004) + (1|Mo), 
                weights = nT, 
                data = df)
summary(model0a)
AIC(model0a)
...
model0c <- lmer(log(GroupDayTotalED) ~ I(mCAPE/1000) * I(mCIN/100) + I(mUSTM/10) + I(mHLCY/100) + I(Yr - 2004) + (1 |Mo), 
                weights = nT, 
                data = df)
summary(model0c)
AIC(model0c)
