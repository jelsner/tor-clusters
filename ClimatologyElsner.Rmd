...
#githubinstall("patchwork")   From: thomasp85/patchwork
library(patchwork)
p2 + p1 + plot_layout(ncol = 1, heights = c(1, 6))
```

Locate specific values on the plot.
```{r}
# CIN at 250 J/kg
...
Models for casualties. NOT USE.
```{r}
model1b <- lmer(log(GroupDayCas + 1) ~ (1|Mo) +
                mCAPE2 +  mBS2,
                weights = nT, 
                data = df)
summary(model1b)
...
Observed versus prediction plot for casualties. NOT USE.
```{r}
